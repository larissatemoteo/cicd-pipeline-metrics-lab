...
def get_test_metrics(
    ...
    target = next((a for a in artifacts if not a.get("expired")), None)
    if target is None:
        return empty
    dl = session.get(target["archive_download_url"], timeout=TIMEOUT)
    if dl.status_code != 200:
        return empty
    try:
        with zipfile.ZipFile(io.BytesIO(dl.content)) as zf:
            name = next(
                (n for n in zf.namelist() if n.endswith("pipeline-metrics.json")), None
            )
            if name is None:
                return empty
